...
def _get_semantic_scores(task: str, root_dir: str, top_k: int = 30) -> dict[str, float]:
    ...
    try:
        from arc.tools.indexer import _update_index, _init_client, _collection

        _init_client()
        _update_index(root_dir)

        from arc.tools.indexer import _collection as collection
...
def _get_structural_scores(top_files: list[str], root_dir: str) -> dict[str, float]:
    ...
    try:
        from arc.tools.ast_tools import find_dependencies, _resolve_import_to_file, _find_project_root
    except ImportError:
        return {}
